...
export const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-32 px-4 md:px-8 max-w-7xl mx-auto relative z-10 overflow-hidden">
      ...
      >
        ...
        >
          ...
          {/* Interactive Category Filter Pills (Mobile Swipeable) */}
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`touch-target px-3.5 py-1.5 sm:px-5 sm:py-2.5 rounded-full text-xs sm:text-sm font-body font-bold transition-all flex items-center gap-1.5 sm:gap-2 border whitespace-nowrap shrink-0 ${
                    isActive
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 sm:gap-4 pt-1 sm:pt-2">
            {currentSkills.map((skill, index) => (
              <motion.div
                ...
                className="px-3 py-2.5 sm:px-5 sm:py-4 rounded-2xl bg-bg/85 border border-white/20 hover:border-[#89AACC] transition-all flex flex-col items-center justify-center text-center space-y-0.5 sm:space-y-1 group shadow-md"
              >
                <span className="text-[10px] sm:text-xs font-body uppercase font-bold text-[#89AACC] tracking-wider opacity-80">
                  {skill.cat}
                </span>
                <span className="text-xs sm:text-base font-body font-bold text-text-primary group-hover:text-white transition-colors truncate w-full">
                  {skill.name}
                </span>
              </motion.div>
            ))}
